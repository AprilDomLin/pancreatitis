...
def main():
    ...
    save_dir = Path(config.save_dir).joinpath(f'BTI_{config.epoch:02d}')
    save_dir.mkdir(parents=True, exist_ok=True)

    # BTI
    bt = it.BTCVInterpreter(model, device='gpu:0')
    for col in dataset_eval.itertuples():
        save_path = save_dir.joinpath(Path(col.path).name)
        bt.interpret(str(col.path),
                    start_layer=config.start_layer,
                    label=int(col.cls),
                    visual=False,
                    attn_map_name=config.attn_map_name,
                    attn_v_name=config.attn_v_name,
                    attn_proj_name=config.attn_proj_name,
                    save_path=save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attn_map_name', type=str, default='^base.blocks.[0-9]*.attn.attn_drop$')
    parser.add_argument('--attn_v_name', type=str, default='^base.blocks.[0-9]*.attn.qkv$')
    parser.add_argument('--attn_proj_name', type=str, default='^base.blocks.[0-9]*.attn.proj$')
